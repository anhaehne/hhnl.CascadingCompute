...
public sealed class CascadingComputeControllerGenerator : IIncrementalGenerator
{
    ...
    private const string AspNetRouteAttributeMetadataName = "Microsoft.AspNetCore.Mvc.RouteAttribute";
    ...
    private static string GenerateControllerSource(INamedTypeSymbol controller)
    {
        ...
        foreach (var containingType in GetContainingTypes(controller))
        {
            ...
            {
                if (serviceInterface is not null && !HasApiControllerAttribute(controller))
                {
                    sb.Append(indent);
                    sb.AppendLine("[global::Microsoft.AspNetCore.Mvc.ApiControllerAttribute]");
                }

                var controllerRoute = serviceInterface is not null ? GetControllerRouteTemplate(serviceInterface) : null;
                if (!string.IsNullOrWhiteSpace(controllerRoute) && !HasAspNetRouteAttribute(controller))
                {
                    sb.Append(indent);
                    sb.Append("[global::Microsoft.AspNetCore.Mvc.RouteAttribute(\"");
                    sb.Append(EscapeStringLiteral(controllerRoute!));
                    sb.AppendLine("\")]");
                }

                if (!HasTypeFilterAttribute(controller))
                {
                    sb.Append(indent);
    ...
    private static string GetHttpMethodAttributeTypeName(IMethodSymbol serviceMethod)
    {
        ...
        if (routeAttribute?.AttributeClass?.ToDisplayString() == RoutePostAttributeMetadataName)
            return "HttpPostAttribute";

        if (routeAttribute?.AttributeClass?.ToDisplayString() == RoutePutAttributeMetadataName)
            return "HttpPutAttribute";

        if (routeAttribute?.AttributeClass?.ToDisplayString() == RouteDeleteAttributeMetadataName)
            return "HttpDeleteAttribute";

        if (routeAttribute?.AttributeClass?.ToDisplayString() == RouteHeadAttributeMetadataName)
            return "HttpHeadAttribute";

        if (routeAttribute?.AttributeClass?.ToDisplayString() == RoutePatchAttributeMetadataName)
            return "HttpPatchAttribute";

        if (routeAttribute?.AttributeClass?.ToDisplayString() == RouteOptionsAttributeMetadataName)
            return "HttpOptionsAttribute";
    ...
    private static bool HasApiControllerAttribute(INamedTypeSymbol controller)
        => controller.GetAttributes().Any(a => a.AttributeClass?.ToDisplayString() == ApiControllerAttributeMetadataName);

    private static bool HasAspNetRouteAttribute(INamedTypeSymbol controller)
        => controller.GetAttributes().Any(a => a.AttributeClass?.ToDisplayString() == AspNetRouteAttributeMetadataName);
